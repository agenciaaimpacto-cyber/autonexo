...
2. **La imagen de portada del home (`assets/hero-home.jpg`) y los 8 flyers de portada de cada auto (`vehiculos/<Ciudad>/<Auto>/portada.png`) siguen mostrando "AUTODIRECTO" escrito dentro de la imagen** — son gráficas ya diseñadas (probablemente con el mismo flujo de ChatGPT/IA que usó Danny para el logo original), no texto editable por HTML/CSS. Hay que regenerarlas con el nombre nuevo — mismo proceso que usó para crear las originales — y reemplazar los archivos correspondientes; el sitio las va a tomar automáticamente al correr `generar_vitrina.py` de nuevo (para los flyers) o con un simple reemplazo de archivo + reoptimizar (para el hero).
3. **Logo definitivo:** el monograma "AN" actual es solo un placeholder — si Danny quiere un logo con el mismo nivel de detalle que el de AutoDirecto (círculo 3D, auto estilizado, etc.), tiene que encargarlo de nuevo (mismo proceso que la vez anterior) y pasarlo para reemplazar `assets/logo-autonexo*.png`.
